...
//taken from http://mrl.nyu.edu/~perlin/noise/
...
namespace gsgeom
{
	...
	{
	public:
		gsnoise (const int &seed=0)
		{
			int p[] = { 151,160,137,91,90,15,131,13, 201,95,96,53,194,233,7,225, 140,36,103,30,69,
		...
		double noise (const gsgeom::gsvec3f &pos)
		{
			int X = (int)floor (pos[0]) & 255,
				Y = (int)floor (pos[1]) & 255,
				Z = (int)floor (pos[2]) & 255;
			float x = pos[0] - floor (pos[0]);
			float y = pos[1] - floor (pos[1]);
			float z = pos[2] - floor (pos[2]);
		...
		double noise (const gsgeom::gsvec3f &pos, const int &iMin, const int &iMax)
		{
			return 0.0;
		}

		double noise (const gsgeom::gsvec3f &pos, const double &fMin, const double &fMax)
		{
			return 0.0;
		}
